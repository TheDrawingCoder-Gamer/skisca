...
public class WatchesScene extends Scene {
    ...
    public void draw(Canvas canvas, int width, int height, float dpi, int xpos, int ypos) {
        var watchFill = Paint.apply().setColor(0xFFFFFFFF);
        var watchStroke = Paint.apply().setColor(0xFF000000).setMode(PaintMode.STROKE).setStrokeWidth(1f).setAntiAlias(false);
        var watchStrokeAA = Paint.apply().setColor(0xFF000000).setMode(PaintMode.STROKE).setStrokeWidth(1f);
        var watchFillHover = Paint.apply().setColor(0xFFE4FF01);
