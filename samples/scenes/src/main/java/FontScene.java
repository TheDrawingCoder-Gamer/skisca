...
public class FontScene extends Scene {
    ...
    public FontScene() {
        ...
        _paint          = Paint.apply().setColor(0xFF000000);
        _stroke         = Paint.apply().setColor(0xFF2a9d8f).setMode(PaintMode.STROKE).setStrokeWidth(2).setPathEffect(PathEffect.makeDash(new float[] {6, 2}, 0));
        _boundaryPaint  = Paint.apply().setColor(0xFFe76f51).setMode(PaintMode.STROKE);
    }
    ...
    public void drawInter(Canvas canvas, float dpi) {
        ...
        for (int i = 0; i < glyphs.length; ++i) {
            canvas.drawRect(Rect.makeXYWH(xPositions[i] + boundRects[i].getLeft(), -metrics.bottom() + boundRects[i].getTop(), boundRects[i].getWidth(), boundRects[i].getHeight()), _boundaryPaint);
        }
