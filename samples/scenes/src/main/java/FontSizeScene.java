...
public class FontSizeScene extends Scene {
    Paint grayStroke  = Paint.apply().setColor(0x40808080).setMode(PaintMode.STROKE).setStrokeWidth(1);
    Paint redStroke   = Paint.apply().setColor(0x80CC3333).setMode(PaintMode.STROKE).setStrokeWidth(1);
    Paint greenStroke = Paint.apply().setColor(0x8033CC33).setMode(PaintMode.STROKE).setStrokeWidth(1);
    Paint blueStroke  = Paint.apply().setColor(0x403333CC).setMode(PaintMode.STROKE).setStrokeWidth(1);

    Paint grayFill  = Paint.apply().setColor(0xFF808080);
    Paint redFill   = Paint.apply().setColor(0xFFCC3333);
    Paint greenFill = Paint.apply().setColor(0xFF33CC33);
    Paint blueFill  = Paint.apply().setColor(0xFF3333CC);
    ...
    public float capHeight(Font font) {
        if ("Ubuntu Mono".equals(font.getTypeface().getFamilyName()))
            ...
            return font.getMetrics().capHeight();
    }
    ...
    public void draw(Canvas canvas, int width, int height, float dpi, int xpos, int ypos) {
        ...
        for (var font: fonts) {
            if ("Cap Height".equals(_variants[_variantIdx])) {
                ...
                var xHeight = font.getMetrics().xHeight();
            ...
            try (var line = TextLine.make("Andy", font);) {
                canvas.drawLine(x - 10, font.getMetrics().ascent(), x + 90, font.getMetrics().ascent(), greenStroke);
                canvas.drawLine(x - 10, -capHeight(font), x + 90, -capHeight(font), redStroke);
                canvas.drawLine(x - 10, -font.getMetrics().xHeight(), x + 90, -font.getMetrics().xHeight(), blueStroke);
                canvas.drawLine(x - 10, 0, x + 90, 0, grayStroke);
                canvas.drawLine(x - 10, font.getMetrics().descent(), x + 90, font.getMetrics().descent(), greenStroke);
                ...
                canvas.drawString(String.format("%.2f", (-font.getMetrics().ascent())), x, y += 15, inter9, greenFill);
                canvas.drawString(String.format("%.2f", capHeight(font)), x, y += 15, inter9, redFill);
                canvas.drawString(String.format("%.2f", font.getMetrics().xHeight()),   x, y += 15, inter9, blueFill);
                canvas.drawString(String.format("%.2f", font.getMetrics().descent()),   x, y += 15, inter9, greenFill);
