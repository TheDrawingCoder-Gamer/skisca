...
object EncoderWEBP {
  ...
  @Nullable def encode(@NotNull image: Image): Data = {
    encode(null, image, EncodeWEBPOptions.DEFAULT)
  }
   ...
   * @return nullptr if the pixels could not be read or encoding otherwise fails.
   */
  @Nullable def encode(@NotNull image: Image, @NotNull opts: EncodeWEBPOptions): Data = {
   ...
   * @return nullptr if the pixels could not be read or encoding otherwise fails.
   */
  @Nullable def encode(@Nullable ctx: DirectContext, @NotNull image: Image, @NotNull opts: EncodeWEBPOptions): Data = {
    try {
      assert(image != null, "Can’t EncoderWEBP::encode with image == null")
      assert(opts != null, "Can’t EncoderWEBP::encode with opts == null")
      Stats.onNativeCall()
      val ptr = _nEncode(Native.getPtr(ctx), Native.getPtr(image), opts.compressionMode.ordinal, opts.quality)
      if (ptr == 0) {
        null
      } else {
        new Data(ptr)
  ...
  @ApiStatus.Internal
  @native def _nEncode(ctxPtr: Long, imagePtr: Long, compressionMode: Int, quality: Float): Long
}